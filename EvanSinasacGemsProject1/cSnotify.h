...
public:
	...
	bool GetSnotifySongs(cSong*& pAllTheSongs, unsigned int& sizeOfSongArray);

private:
	// Need some kind of map to track each user's list of songs
	// I think the idea is to have some kind of list of users
	// as well as a list of all songs
	// but then each user is going to have a playlist (since this IS Snotify)
	// and all these lists need to be sorted, and what they return need to be sorted
	//cMap_List m_mUserSongs;
	cSmartArray_Template<cPerson*> m_saUsers;
	cSmartArray_Template<cSong*> m_saSongs;
};
